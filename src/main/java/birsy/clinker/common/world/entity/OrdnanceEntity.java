...
public class OrdnanceEntity extends Projectile {
    ...
    private static final EntityDataAccessor<Boolean> DATA_DEFLECTION = SynchedEntityData.defineId(OrdnanceEntity.class, EntityDataSerializers.BOOLEAN);
    ...
    public void onAddedToWorld() {
        ...
        Clinker.LOGGER.info(this.level().isClientSide);
    ...
    public Vec3 getStrapVector(float partialTicks) {
        return this.previousVelocity.lerp(this.getDeltaMovement(), partialTicks);
    }
    ...
    public static void createOrdnanceExplosion(Vec3 location, Level level, @Nullable Entity thrower, @Nullable OrdnanceEntity ordnance) {
        ...
        if (level.isClientSide) {
            ...
            ClinkerPacketHandler.sendToClientsInChunk(level.getChunkAt(BlockPos.containing(location)), new ClientboundOrdnanceExplosionPacket(location));
        }
        ...
        for (Entity entity : level.getEntities(ordnance, new AABB(location.subtract(radius, radius, radius), location.add(radius, radius, radius)))) {
            ...
            entity.hurt(entity.damageSources().explosion(ordnance, thrower), damage);
            if (entity instanceof LivingEntity l) {
                if (l.isDamageSourceBlocked(entity.damageSources().explosion(ordnance, thrower))) {

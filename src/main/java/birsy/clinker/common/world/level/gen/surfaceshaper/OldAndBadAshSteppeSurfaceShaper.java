...
public class OldAndBadAshSteppeSurfaceShaper implements SurfaceShaper {
    ...
    public double surfaceDensity(int x, int y, int z, double biomeContribution, NoiseComputerContext context) {
        ...
        double yScale = 0.2F;
        double scaledSurfaceHeight = (cache.compute(x, y, z, OthershoreNoiseComputers.SURFACE_HEIGHT_COMPUTER) - 10) * yScale;
        double scaledY = y * yScale;

        if (scaledY - scaledSurfaceHeight > 2.0F / heightFactor) return 10.0F;

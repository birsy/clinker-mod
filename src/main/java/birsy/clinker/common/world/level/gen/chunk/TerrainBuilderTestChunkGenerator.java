...
public class TerrainBuilderTestChunkGenerator extends ChunkGenerator implements LevelNoiseProvidable {
    ...
    public CompletableFuture<ChunkAccess> fillFromNoise(Blender densityBlender, RandomState random, StructureManager structureManager, ChunkAccess chunk) {
        ...
        noiseField.fill((currentValue, x, y, z, params) -> (float) MathUtil.smoothMinExpo(sampleCaveNoise(x, y, z), currentValue, 0.05F));
    ...
    public float sampleCaveNoise(double x, double y, double z) {
        ...
        return (float) MathUtil.smoothMinExpo(MathUtil.smoothMinExpo(sCave, stalag, -0.05) + Math.max(bCaveShell * 0.8, 0), bCave, 0.06);//MathUtils.smoothMinExpo(stalag, sampleBridgeCaveNoise(x, y, z, seed), -0.05);//MathUtils.smoothMinExpo(aCave, MathUtils.smoothMinExpo(sCave, stalag, -0.1), 0.06);
    }
    ...
    public static void register() {
        Registry.register(BuiltInRegistries.CHUNK_GENERATOR, Clinker.resource("terrain_builder_test_chunk_generator"), CODEC);
    }

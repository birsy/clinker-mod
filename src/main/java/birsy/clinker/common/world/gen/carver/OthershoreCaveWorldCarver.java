...
public class OthershoreCaveWorldCarver extends WorldCarver<ProbabilityConfig> {
	public OthershoreCaveWorldCarver(Codec<ProbabilityConfig> p_i231917_1_, int p_i231917_2_) {
	...
	public boolean carveRegion(IChunk chunk, Function<BlockPos, Biome> biomePos, Random rand, int seaLevel, int chunkXOffset, int chunkZOffset, int chunkX, int chunkZ, BitSet carvingMask, ProbabilityConfig config) {
		...
		for(int k = 0; k < j; ++k) {
			...
			for(int k1 = 0; k1 < l; ++k1) {
	            ...
	            this.func_227206_a_(chunk, biomePos, rand.nextLong(), seaLevel, chunkX, chunkZ, d0, d1, d2, f2 * 2.0F, f, f3, 0, i1, this.func_230360_b_(), carvingMask);
			}
		}
		return true;
	}
	...
	public Set<Block> carvableBlocks = ImmutableSet.of(ClinkerBlocks.ASH.get(), ClinkerBlocks.PACKED_ASH.get(), ClinkerBlocks.BRIMSTONE.get(), ClinkerBlocks.COBBLED_BRIMSTONE.get(), ClinkerBlocks.SHALE.get());
	
	@Override
	protected boolean isCarvable(BlockState blockStateIn) {
		return this.carvableBlocks.contains(blockStateIn.getBlock());
	}

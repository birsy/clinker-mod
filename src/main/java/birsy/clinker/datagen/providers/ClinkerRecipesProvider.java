...
import birsy.clinker.datagen.custom.MortarAndPestleRecipeBuilder;
...
public class ClinkerRecipesProvider extends RecipeProvider {
    public ClinkerRecipesProvider(PackOutput output, CompletableFuture<HolderLookup.Provider> registries) {
    ...
    protected void buildRecipes(RecipeOutput output) {
        ...
        recipe("gunpowder_grinding", output,
                new MortarAndPestleRecipeBuilder(Items.GUNPOWDER.getDefaultInstance().copyWithCount(2))
                .ingredients(
                        Ingredient.of(ClinkerItems.SULFUR.get()),
                        Ingredient.of(ClinkerItems.SALTPETRE.get())
                )
        );
    }

    protected static void recipe(String path, RecipeOutput output, RecipeBuilder recipeBuilder) {
        recipeBuilder.save(output, Clinker.resource(path));
    }

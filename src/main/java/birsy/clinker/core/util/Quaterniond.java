 ...
 * @author Kai Burjack
 */
public class Quaterniond implements Externalizable, Cloneable {
    ...
    public Quaterniond(Vec3 axis, double angle) {
    ...
    public Quaterniond(AxisAngled aa) {
        double s = Math.sin(aa.angle * 0.5);
        x = aa.x() * s;
        y = aa.y() * s;
        z = aa.z() * s;
        w = Math.cos(aa.angle * 0.5);
    }
    ...
    public Quaterniond normalize() {
        double invNorm = 1 / Math.sqrt(lengthSquared());
        x *= invNorm;
        y *= invNorm;
        z *= invNorm;
        w *= invNorm;
    ...
    public AxisAngled get() {
        ...
        if (s < 0.001) {
            return new AxisAngled(new Vec3(x, y, z), (2.0 * Math.acos(w)));
        } else {
            s = 1.0 / s;
            return new AxisAngled(new Vec3(x * s, y * s, z * s), (2.0 * Math.acos(w)));
        }
    }
     ...
     * @return this
     */
    public Quaterniond set(AxisAngled axisAngle) {
     ...
     * @return this
     */
    public Quaterniond setAngleAxis(double angle, Vec3 axis) {
    ...
    private void setFromUnnormalized(double m00, double m01, double m02, double m10, double m11, double m12, double m20, double m21, double m22) {
        ...
        double lenX = 1 / Math.sqrt(m00 * m00 + m01 * m01 + m02 * m02);
        double lenY = 1 / Math.sqrt(m10 * m10 + m11 * m11 + m12 * m12);
        double lenZ = 1 / Math.sqrt(m20 * m20 + m21 * m21 + m22 * m22);
    ...
    public Vec3 transformPositiveX() {
        double ww = w * w;
        double xx = x * x;
        double yy = y * y;
        double zz = z * z;
        ...
        double xz = x * z;
        double yw = y * w;
        return new Vec3(ww + xx - zz - yy, xy + zw + zw + xy, xz - yw + xz - yw);
    }

    public Vec3 transformUnitPositiveX() {
        double yy = y * y;
        double zz = z * z;
        double xy = x * y;
        double xz = x * z;
        double yw = y * w;
        double zw = z * w;
        return new Vec3(1.0 - yy - yy - zz - zz, xy + zw + xy + zw, xz - yw + xz - yw);
    }
    
    public Vec3 transformPositiveY() {
        double ww = w * w;
        double xx = x * x;
        double yy = y * y;
        double zz = z * z;
        ...
        double yz = y * z;
        double xw = x * w;
        return new Vec3(-zw + xy - zw + xy, yy - zz + ww - xx, yz + yz + xw + xw);
    }
    
    public Vec3 transformUnitPositiveY() {
        double xx = x * x;
        double zz = z * z;
        double xy = x * y;
        double yz = y * z;
        double xw = x * w;
        double zw = z * w;
        double nx = xy - zw + xy - zw;
        double ny = 1.0 - xx - xx - zz - zz;
        double nz = yz + yz + xw + xw;
        return new Vec3(nx, ny, nz);
    }

    public Vec3 transformPositiveZ() {
        double ww = w * w;
        double xx = x * x;
        double yy = y * y;
        double zz = z * z;
        ...
        double yz = y * z;
        double xw = x * w;
        double nx = yw + xz + xz + yw;
        double ny = yz + yz - xw - xw;
        double nz = zz - yy - xx + ww;
        return new Vec3(nx, ny, nz);
    }

    public Vec3 transformUnitPositiveZ() {
        double xx = x * x;
        double yy = y * y;
        double xz = x * z;
        double yz = y * z;
        double xw = x * w;
        double yw = y * w;
        double nx = xz + yw + xz + yw;
        double ny = yz + yz - xw - xw;
        double nz = 1.0 - xx - xx - yy - yy;
        return new Vec3(nx, ny, nz);
    }
    ...
    public Vec3 transformInverse(Vec3 vec) {
    ...
    public Vec3 transformInverse(double x, double y, double z) {
    ...
    public Vec3 transformUnit(Vec3 vec) {
        return transformUnit(vec.x(), vec.y(), vec.z());
    }

    public Vec3 transformInverseUnit(Vec3 vec) {
        return transformInverseUnit(vec.x(), vec.y(), vec.z());
    }

    public Vec3 transformUnit(double x, double y, double z) {
        ...
        return new Vec3(Math.fma(Math.fma(-2, yy + zz, 1), x, Math.fma(2 * (xy - zw), y, (2 * (xz + yw)) * z)),
                Math.fma(2 * (xy + zw), x, Math.fma(Math.fma(-2, xx + zz, 1), y, (2 * (yz - xw)) * z)),
                Math.fma(2 * (xz - yw), x, Math.fma(2 * (yz + xw), y, Math.fma(-2, xx + yy, 1) * z)));
    }

    public Vec3 transformInverseUnit(double x, double y, double z) {
        double xx = this.x * this.x, xy = this.x * this.y, xz = this.x * this.z;
        double xw = this.x * this.w, yy = this.y * this.y, yz = this.y * this.z;
        double yw = this.y * this.w, zz = this.z * this.z, zw = this.z * this.w;
        return new Vec3(Math.fma(Math.fma(-2, yy + zz, 1), x, Math.fma(2 * (xy + zw), y, (2 * (xz - yw)) * z)),
    ...
    public Quaterniond slerp(Quaterniond target, double alpha, Quaterniond dest) {
        ...
        if (1.0 - absCosom > 1E-6) {
            ...
            double sinom = 1 / Math.sqrt(sinSqr);
    ...
    public Quaterniond nlerp(Quaterniond q, double factor, Quaterniond dest) {
        ...
        double s = 1 / Math.sqrt(Math.fma(dest.x, dest.x, Math.fma(dest.y, dest.y, Math.fma(dest.z, dest.z, dest.w * dest.w))));
        dest.x *= s;
        dest.y *= s;
        dest.z *= s;
        dest.w *= s;
    ...
    public Quaterniond nlerpIterative(Quaterniond q, double alpha, double dotThreshold, Quaterniond dest) {
        ...
        while (absDot < dotThreshold) {
            ...
            if (alphaN < 0.5) {
                ...
                float s = (float) ((float) 1 / Math.sqrt(Math.fma(q2x, q2x, Math.fma(q2y, q2y, Math.fma(q2z, q2z, q2w * q2w)))));
                q2x *= s;
                q2y *= s;
                q2z *= s;
                q2w *= s;
                ...
                float s = (float) ((float) 1 / Math.sqrt(Math.fma(q1x, q1x, Math.fma(q1y, q1y, Math.fma(q1z, q1z, q1w * q1w)))));
                q1x *= s;
                q1y *= s;
                q1z *= s;
                q1w *= s;
        ...
        double s = 1 / Math.sqrt(Math.fma(resX, resX, Math.fma(resY, resY, Math.fma(resZ, resZ, resW * resW))));
        dest.x = resX * s;
        dest.y = resY * s;
        dest.z = resZ * s;
        dest.w = resW * s;
     ...
     * @return this
     */
    public Quaterniond lookAlong(Vec3 dir, Vec3 up) {
        return lookAlong(dir.x(), dir.y(), dir.z(), up.x(), up.y(), up.z(), this);
    }

    public Quaterniond lookAlong(Vec3 dir, Vec3 up, Quaterniond dest) {
    ...
    public Quaterniond lookAlong(double dirX, double dirY, double dirZ, double upX, double upY, double upZ, Quaterniond dest) {
        // Normalize direction
        double invDirLength = 1 / Math.sqrt(dirX * dirX + dirY * dirY + dirZ * dirZ);
        ...
        // normalize left
        double invLeftLength = 1 / Math.sqrt(leftX * leftX + leftY * leftY + leftZ * leftZ);
        leftX *= invLeftLength;
        leftY *= invLeftLength;
        leftZ *= invLeftLength;
        // up = direction x left
    ...
    public Quaterniond rotationTo(double fromDirX, double fromDirY, double fromDirZ, double toDirX, double toDirY, double toDirZ) {
        double fn = 1 / Math.sqrt(Math.fma(fromDirX, fromDirX, Math.fma(fromDirY, fromDirY, fromDirZ * fromDirZ)));
        double tn = 1 / Math.sqrt(Math.fma(toDirX, toDirX, Math.fma(toDirY, toDirY, toDirZ * toDirZ)));
        ...
        if (dot < -1.0 + 1E-6) {
            ...
            z = cz * isd2;
            w = sd2 * 0.5;
            double n2 = 1 / Math.sqrt(Math.fma(x, x, Math.fma(y, y, Math.fma(z, z, w * w))));
            this.x = x * n2;
            this.y = y * n2;
            this.z = z * n2;
            this.w = w * n2;
     ...
     * @return this
     */
    public Quaterniond rotationTo(Vec3 fromDir, Vec3 toDir) {
     ...
     * axis, all of which are specified via the {@link AxisAngled}.
     *
     * @see #rotationAxis(double, double, double, double)
     *
     * @param axisAngle
     *            the {@link AxisAngled} giving the rotation angle in radians and the axis to rotate about
     * @return this
     */
    public Quaterniond rotationAxis(AxisAngled axisAngle) {
    ...
    public Quaterniond rotationAxis(double angle, double axisX, double axisY, double axisZ) {
        ...
        double invVLength = 1 / Math.sqrt(axisX * axisX + axisY * axisY + axisZ * axisZ);
    ...
    public Quaterniond rotateTo(Vec3 fromDir, Vec3 toDir, Quaterniond dest) {
     ...
     * @return this
     */
    public Quaterniond rotateTo(Vec3 fromDir, Vec3 toDir) {
    ...
    public Vec3 getEulerAnglesXYZ(Vec3 eulerAngles) {
        double ex = Math.atan2(x * w - y * z, 0.5 - x * x - y * y);
        double ey = Math.asin(2.0 * (x * z + y * w));
        double ez = Math.atan2(z * w - x * y, 0.5 - y * y - z * z);
    ...
    public Quaterniond rotateAxis(double angle, double axisX, double axisY, double axisZ, Quaterniond dest) {
        double hangle = angle / 2.0;
        double sinAngle = Math.sin(hangle);
        double invVLength = 1 / Math.sqrt(Math.fma(axisX, axisX, Math.fma(axisY, axisY, axisZ * axisZ)));
    ...
    public Quaterniond rotateAxis(double angle, Vec3 axis, Quaterniond dest) {
     ...
     * @return this
     */
    public Quaterniond rotateAxis(double angle, Vec3 axis) {
    ...
    public Vec3 positiveX(Vec3 dir) {
        double invNorm = 1.0 / lengthSquared();
        double nx = -x * invNorm;
        double ny = -y * invNorm;
        double nz = -z * invNorm;
        ...
        double dy = ny + ny;
        double dz = nz + nz;
        double dix = -ny * dy - nz * dz + 1.0;
        double diy =  nx * dy + nw * dz;
        double diz =  nx * dz - nw * dy;
        return new Vec3(dix, diy, diz);
    }

    public Vec3 normalizedPositiveX(Vec3 dir) {
        double dy = y + y;
        double dz = z + z;
        double dix = -y * dy - z * dz + 1.0;
        double diy =  x * dy - w * dz;
        double diz =  x * dz + w * dy;
        return new Vec3(dix, diy, diz);
    }

    public Vec3 positiveY(Vec3 dir) {
        double invNorm = 1.0 / lengthSquared();
        double nx = -x * invNorm;
        double ny = -y * invNorm;
        double nz = -z * invNorm;
        ...
        double dy = ny + ny;
        double dz = nz + nz;
        double dix =  nx * dy - nw * dz;
        double diy = -nx * dx - nz * dz + 1.0;
        double diz =  ny * dz + nw * dx;
        return new Vec3(dix, diy, diz);
    }

    public Vec3 normalizedPositiveY(Vec3 dir) {
        double dx = x + x;
        double dy = y + y;
        double dz = z + z;
        double dix =  x * dy + w * dz;
        double diy = -x * dx - z * dz + 1.0;
        double diz =  y * dz - w * dx;
        return new Vec3(dix, diy, diz);
    }

    public Vec3 positiveZ(Vec3 dir) {
        double invNorm = 1.0 / lengthSquared();
        double nx = -x * invNorm;
        double ny = -y * invNorm;
        double nz = -z * invNorm;
        ...
        double dy = ny + ny;
        double dz = nz + nz;
        double dix =  nx * dz + nw * dy;
        double diy =  ny * dz - nw * dx;
        double diz = -nx * dx - ny * dy + 1.0;
        return new Vec3(dix, diy, diz);
    }

    public Vec3 normalizedPositiveZ(Vec3 dir) {
        double dx = x + x;
        double dy = y + y;
        double dz = z + z;
        double dix =  x * dz - w * dy;
        double diy =  y * dz + w * dx;
        double diz = -x * dx - y * dy + 1.0;
        return new Vec3(dix, diy, diz);
    }

...
public class VerletRigidBody implements ICollidable, IBody, IPhysicsBody, ITickableBody {
    ...
    public void rotate(Vec3 rot, double scale) {
        Quaterniond q = new Quaterniond((float) (rot.x * scale), (float) (rot.y * scale), (float) (rot.z * scale), 0.0F);
        Quaterniond orientation = this.transform.getOrientation();
        q.mul(orientation);
        orientation.set((float) (orientation.x() + 0.5 * q.x()), (float) (orientation.y() + 0.5 * q.y()), (float) (orientation.z() + 0.5 * q.z()), (float) (orientation.w() + 0.5 * q.w()));
